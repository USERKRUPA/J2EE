...
public class UserDaoImpl implements UserDao {
	...
	public UserDaoImpl() throws SQLException {
		...
		pst6 = connection.prepareStatement(" select *  from candidates where votes in (select max(votes) from candidates) ");
		pst7 = connection.prepareStatement(
				"select *  from candidates where votes in (select max(votes) from candidates where votes < (select max(votes) from candidates))");
...
//TO DO : validate user role : voter 
	...
	@Override
	public List<Candidate> getHighestCandidate() throws SQLException {
		List<Candidate> list = new ArrayList<Candidate>();
		try (ResultSet rs = pst6.executeQuery(); ResultSet rst2 = pst7.executeQuery()) {
			while (rs.next())
				list.add(new Candidate(rs.getString(2)));

			while (rst2.next())
				list.add(new Candidate(rst2.getString(2)));
